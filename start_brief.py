...
# 카테고리명 (한곳에 모음 — 콘솔/Excel 공통)
CAT_SELL = 'profit taking (거래량 동반한 이탈)'   # 10일선 하회 + 거래량 폭증
CAT_CAUTION = 'profit taking (단순 이탈)'         # 10일선 하회 + 거래량 부족
CAT_UPSIDE = 'upside'                            # 10일선 위 + 거래량 폭증

# Excel 노이즈 필터 (profit taking 시그널O/X 에만 적용)
...
# 미국 티커는 'AAPL US'처럼 US를 명시한 경우만 인정
# (TIME/SOL/ACE 등 한국 ETF 브랜드명 조각이 실존 미국 티커와 겹쳐 오인되는 것 방지)
_US_EXPLICIT = re.compile(r'\b([A-Z]{1,5})\s+US\b', re.IGNORECASE)
...
def parse_ticker_input(raw: str, master: dict = None, verify_unknown: bool = True) -> tuple:
    ...
    us_tokens = [m.group(1).upper() for m in _US_EXPLICIT.finditer(s)]
...
def print_console(results: list, names: dict) -> pd.DataFrame:
    ...
    show(CAT_SELL, '10일선 하회 + 거래량 폭증 (SELL)', sell)
    show(CAT_CAUTION, '10일선 하회 + 거래량 부족 (HOLD)', caution)
    show(CAT_UPSIDE, '10일선 위 + 거래량 폭증 (BUY)', upside)
    return df
...
def save_excel(df: pd.DataFrame, names: dict, mode: int) -> str:
    sell = [_row(CAT_SELL, r, names) for _, r in df[df['signal'] == 'SELL'].iterrows()]
    caution = [_row(CAT_CAUTION, r, names) for _, r in
               df[df['condition_1_trend_breakdown'] & ~df['condition_2_volume_confirmation']].iterrows()]
    upside = [_row(CAT_UPSIDE, r, names) for _, r in
              df[~df['condition_1_trend_breakdown'] & df['condition_2_volume_confirmation']].iterrows()]
    ...
    print(f"  - {CAT_SELL}: {len(sell)} → {len(sell_f)}")
    print(f"  - {CAT_CAUTION}: {len(caution)} → {len(caution_f)}")
    print(f"  - {CAT_UPSIDE}: {len(upside)} (필터 제외)")
    ...
    order = {CAT_SELL: 0, CAT_CAUTION: 1, CAT_UPSIDE: 2}
    ...
    with pd.ExcelWriter(path, engine='openpyxl') as w:
        ...
        if sell_f:
            sort_part(sell_f).to_excel(w, sheet_name='거래량 동반한 이탈', index=False)
        if caution_f:
            sort_part(caution_f).to_excel(w, sheet_name='단순 이탈', index=False)
        if upside:
